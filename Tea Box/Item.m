...
{
	__block NSMutableArray * items = [NSMutableArray arrayWithCapacity:10];
...
- (void)updateRowIndex:(int)rowIndex
{
	...
	if (_step.project.library.database) {
		[SandboxHelper executeWithSecurityScopedAccessToPath:_step.project.library.path block:^(NSError * error) {
			if (!error) {
